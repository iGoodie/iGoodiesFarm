...
public class BlockBlueberryBush extends BlockLeavesBase implements IPlantable, IGrowable
{
	public IIcon[] icons;
	...
	{	
		...
		for(int i=0; i<this.icons.length; i++)
		{
			if(i!=7)
				...
				this.icons[i] = iconRegister.registerIcon("igoodiesfarm:blueberryBush");
		}
	}
	...
	public void onNeighborBlockChange(World world, int x, int y, int z, Block changingBlock) 
	{
		if(world.isAirBlock(x, y-1, z))
		{
			...
			GoodiesFarmBlocks.blockBlueberryBush.dropBlockAsItem(world, x, y, z, new ItemStack(GoodiesFarmBlocks.blockBlueberryBush, 1, 0));
		}
	}
	...
	public void onBlockClicked(World world, int x, int y, int z, EntityPlayer player)
	{
		...
		if(meta == 7)
		{
			...
			player.inventory.addItemStackToInventory(new ItemStack(GoodiesFarmItems.itemBlueberry, world.rand.nextInt(3)+1));
		}
	}
	...
	public void updateTick(World world, int x, int y, int z, Random randomPar)
	{
		...
		if(light>=8)
		{
			...
			if(meta==6 || meta==7)
			{
				if(world.isAirBlock(x, y+1, z) && !(world.getBlock(x, y-2, z) instanceof BlockBlueberryBush) )
				{
					if(randomPar.nextInt(3) == 0)
					{
						world.setBlock(x, y+1, z, GoodiesFarmBlocks.blockBlueberryBush);
					}
				}
			}
		}
	...
	public boolean canPlaceBlockAt(World world, int x, int y, int z)
	{
		return     world.getBlock(x, y-1, z) instanceof BlockGrass 
				...
				|| (world.getBlock(x, y-1, z) instanceof BlockBlueberryBush && world.getBlockMetadata(x, y-1, z)>=6);
	}
